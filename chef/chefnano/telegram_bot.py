...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("Hello! I'm Nano. Use /nano <question> to query the recipe database.")

async def nano_query(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # Before example: "/nano pasta ideas" ignored; After: /nano routes to answer_with_nano.
    user_id = update.effective_user.id
    application_data = context.application
    question = " ".join(context.args or []).strip()
    if not question:
        await update.message.reply_text("Usage: /nano <question>")
        return
    ...
    session_info = {
        ...
        "trigger_command": "/nano",
    }

    with open(f"application_data_for_{session_info['user_id']}.txt", "w") as f:
        f.write(str(application_data))

    message_object = get_user_handler(user_id, session_info, question)
    router_instance_for_this_call = MessageRouter()
    logging.info("nano_query: routing question for user_id=%s", user_id)
...
async def bot_mode_switch_diet_log(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    sys.stdout.flush()
    sys.stderr.flush()
    # Before example: /workspaces/chevdev/chef/chefmain/main.py fails in Cloud Run.
    # After example: /app/chef/chefmain/main.py resolves from this file at runtime.
    chefmain_path = os.path.join(parent_dir, "chefmain", "main.py")
    ...
    logging.info(
        ...
        chefmain_path,
        os.path.exists(chefmain_path),
        os.getcwd(),
        os.environ.get("BOT_MODE"),
    )
    os.execv(sys.executable, [sys.executable, chefmain_path])
...
def setup_bot() -> Application:
    ...
    application.add_handler(CommandHandler("nano", nano_query))
